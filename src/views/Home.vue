        ...
        <py-col :span="24" class="main">
            <aside class="py-col py-col-4">
              ...
              <py-menu :default-active="leftMenuActive" class="py-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect"
                ...
                <template v-if="topMenuActive=='platform'"><!--当为平台管理时-->
                  <template v-for="(item,index) in leftMenus" v-if="!item.hidden">
                    <py-submenu :index="item.name" v-if="!item.leaf">
                      <template slot="title"><i :class="item.iconCls"></i>{{item.menuName}}</template>
                      <py-menu-item v-for="child in item.children" :index="child.name" :key="child.path" @click="$router.push(child.path)" v-if="!child.hidden">{{child.menuName}}</py-menu-item>
                    </py-submenu>
                    <py-menu-item v-if="item.leaf&&item.children.length>0" :index="item.children[0].name" ><i :class="item.iconCls"></i>{{item.children[0].menuName}}</py-menu-item>
                  </template>
                </template>
                <template v-else><!-- 投放管理，报表中心 -->
                  <py-menu-item v-for="(item,index) in leftMenus" v-if="!item.hidden" :index="item.name" :key="item.path" @click="$router.push(item.path)" >{{item.menuName}}</py-menu-item>
                </template>
              </py-menu>
            </aside>
                ...
                <div class="grid-content bg-purple-light">
                  <py-col :span="24" class="breadcrumb-container">
                    <py-breadcrumb separator="/" class="breadcrumb-inner">
                      <py-breadcrumb-item v-for="item,i in breadcrumbs" :to="{ path: item.path }" :key="item.path">
                        {{ item.menuName }}
                      </py-breadcrumb-item>
                    </py-breadcrumb>
